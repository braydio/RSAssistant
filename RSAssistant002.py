...
`bot.command(
    name="tosell",
    help="Usage: `..addsell <ticker>` Add a ticker to the sell list."
)
async def add_to_sell(ctx, ticker: str):
    ticker = ticker.upper()
    watch_list_manager.add_to_sell_list(ticker)
    await ctx.send(fBadded {ticker} to the sell list.)
...
async def remove_sell(ctx, ticker: str):
    ...
    if watch_list_manager.remove_from_sell_list(ticker):
        await ctx.send(fRemoved {ticker} from the sell list.)
    else:
...
bot.command(
    name="selling",
    help="View the current sell list."
)
async def view_sell_list(ctx):
    ...
    if not sell_list:
        ...
        for ticker, details in sell_list.items():
            aded_on = details.get("added_on", "N/A")
