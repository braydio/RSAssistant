def analyze_fractional_share_policy(text: str) -> dict:
    ...
    # detect any mention of fractional shares
    fractional_terms = [
    ...
    # prepare default result when mention exists but handling unclear
    result = {
    ...
    # helper to extract snippet (5 words before and after match)
    def extract_snippet(phrase: str) -> str | None:
        pattern = re.compile(
            rf'(?:\S+\s+){{0,5}}{re.escape(phrase)}(?:\s+\S+){{0,5}}',
            re.IGNORECASE
        )
        m = pattern.search(text)
        return m.group(0).strip() if m else None

    # basic negation check: ensure snippet does not contain 'not' or 'no' immediately before
    def is_negated(snippet: str, phrase: str) -> bool:
        pre = snippet.lower().split(phrase)[0]
        return bool(re.search(r'\b(not|no)\b\s*$', pre))

    # check each handling category in priority order
    for method, variants in [
        ...
        for phrase in variants:
            ...
            if snippet and not is_negated(snippet, phrase):
                ...
                logger.debug(f"Detected {method} via snippet: '{snippet}'")
                return result

    # no explicit handling found, extract snippet around first fractional mention
