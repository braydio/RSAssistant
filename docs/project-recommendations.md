...
2. Split watch state now persists to `volumes/db/split_watchlist.json` (or
   `VOLUMES_DIR/db/split_watchlist.json` when overridden) alongside the other
   runtime artifacts in `volumes/`.
...
Keep `split_watchlist.json` under `VOLUMES_DIR/db/` and document it alongside
...
- Runtime state: Centralizing artifacts like `split_watchlist.json` under
  `VOLUMES_DIR/db/` aligns with the deployed data layout and keeps versioned
  config in `config/`. The split watch file now shares the `volumes/db/`
  persistence layer with the SQLite database, so the docs and loader logic
  should reflect the relocated file.
...
2. Audit `utils/` for any remaining Discord-context logic, migrate those pieces
   ...
   unused utilities.
3. Confirm `split_watchlist.json` references now point to `volumes/db/` (or
   `VOLUMES_DIR/db/`) and that the documentation reflects the new persistence
   path.
