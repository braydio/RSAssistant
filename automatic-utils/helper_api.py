...
def fun_run(orderObj: stockOrder, command, botObj=None, loop=None):
    if command in [("_init", "_holdings"), ("_init", "_transaction")]:
        ...
        if "_holdings" in command:
            printAndDiscord(
                f"DEAD CANARY NOT DEAD BRAYDEN | Total Value of All Accounts: ${format(totalValue, '0.2f')}", loop
            )
...
if __name__ == "__main__":
    ...
    if DISCORD_BOT:
        ...
        print()
        print("DEAD CANARY CHECK BRAYDEN - NOT DEAD | Discord bot is started...")
        print()
        ...
        async def on_ready():
            try:
                ...
                if channel is None:
                    logging.error("Invalid channel ID, please check your DISCORD_CHANNEL in your .env file and try again")
                    os._exit(1)  # Special exit code to restart docker container
                logging.info("Bot is ready and connected to the channel")
                await channel.send("DEAD CANARY CHECK BRAYDEN - CANARY NOT DEAD | Discord bot is started...")
            except Exception as e:
        ...
        async def on_message(message):
            try:
                if message.channel.id == DISCORD_CHANNEL:
                    logging.info(f"Received message: {message.content} from {message.author} (Bot: {message.author.bot})")
                    await bot.process_commands(message)
                    if message.author.bot:
                        logging.info(f"Processing bot message: {message.content}")
                        await bot.process_commands(message)
            except Exception as e:
        ...
        async def rsa(ctx, *args):
            try:
                logging.info(f"RSA command received with arguments: {args}")
                discOrdObj = await bot.loop.run_in_executor(None, argParser, args)
                event_loop = asyncio.get_event_loop()
                if discOrdObj.get_holdings():
                    logging.info("Executing holdings command")
                    ...
                    logging.info("Executing transaction command")
                    await bot.loop.run_in_executor(
                        None,
                        fun_run,
                        discOrdObj,
                ...
                logging.error(f"Error placing order: {err}")
                if ctx:
        ...
        async def on_command_error(ctx, error):
            try:
                ...
                logging.error(f"Error handling command error: {e}")

        # Run Discord bot
        try:
            bot.run(DISCORD_TOKEN)
            logging.info("Discord bot is running...")
        except Exception as e:
            logging.error(f"Error running the bot: {e}")
        print("Dead Canary Check Brayden - This Canary is not Dead!")
        print()
'''